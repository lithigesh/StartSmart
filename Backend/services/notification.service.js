...
class NotificationService {
  ...
  static async createWelcomeNotification(user) {
    try {
      ...
      if (user.role === "investor") {
        ...
        message =
          "Welcome! Start exploring innovative startup ideas and connect with entrepreneurs. Your investment journey begins here.";
        actionUrl = "/investor/dashboard";
      } else if (user.role === "entrepreneur") {
        title = "Welcome to StartSmart - Entrepreneur Dashboard";
        message =
          "Welcome! Ready to pitch your startup idea? Create your first idea and get it analyzed by our AI system.";
        actionUrl = "/entrepreneur/dashboard";
      } else {
        title = "Welcome to StartSmart";
        message =
          "Welcome to the StartSmart platform! Explore the features available to you.";
        actionUrl = "/dashboard";
      }
  ...
  static async createInvestorInterestNotification(
    investorId,
    ideaId,
    entrepreneurId,
    ideaTitle
  ...
  // Generic notification creation method (used by funding controllers)
  static async createNotification(
    ...
    try {
      // Extract title from message or use first 50 chars
      const title =
        options.title || message.substring(0, 50).replace(/[^\w\s]/gi, "");
      ...
      await Notification.create({
        ...
        type: type || "system",
        priority: options.priority || "medium",
        actionUrl: options.actionUrl || `/funding/${relatedId}`,
        relatedIdea: options.relatedIdea || relatedId,
        relatedUser: options.relatedUser,
        expiresAt: options.expiresAt,
      });
    } catch (error) {

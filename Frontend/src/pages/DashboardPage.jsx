  ...
  const dashboardCards = [
    {
      ...
      description:
        user?.role === "entrepreneur"
          ...
          : "Browse innovative startup ideas",
      icon: <FaLightbulb />,
      count: user?.role === "entrepreneur" ? "3" : "142",
    ...
    },
    {
      ...
      icon: <FaBriefcase />,
      count: user?.role === "entrepreneur" ? "$25K" : "$180K",
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center gap-4">
              <div
                className={`w-10 h-10 rounded-lg bg-white/10 flex items-center justify-center ${getRoleColor()}`}
              >
                {getRoleIcon()}
              </div>
              <div>
          ...
          <div className="bg-white/[0.03] backdrop-blur-xl border border-white/10 rounded-2xl p-8 relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-white/70 font-manrope mb-6 max-w-2xl">
                {user?.role === "entrepreneur"
                  ? "Your dashboard to manage ideas, track progress, and connect with investors. Start by submitting your next big idea."
                  : "Explore curated startup ideas, analyze opportunities, and build your investment portfolio with promising entrepreneurs."}
              </p>

              <div className="flex flex-col sm:flex-row gap-4">
                <button className="btn bg-white text-black hover:bg-gray-100 rounded-lg px-6 py-3 font-manrope font-medium transition-all duration-300 hover:scale-105">
                  {user?.role === "entrepreneur"
                    ? "Submit New Idea"
                    : "Browse Ideas"}
                </button>
                <button className="btn btn-outline border-white text-white hover:bg-white hover:text-black rounded-lg px-6 py-3 font-manrope font-medium transition-all duration-300 hover:scale-105">
                  View Analytics
                </button>
              </div>
            </div>
              ...
              {[1, 2, 3].map((item) => (
                ...
                >
                  <div className="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center">
                    <FaBell className="w-4 h-4 text-white/70" />
                  </div>
                  <div className="flex-1">
                    <p className="text-white font-manrope font-medium">
                      {user?.role === "entrepreneur"
                        ? `New investor interest in "AI Healthcare Platform"`
                        : `New idea submitted: "Sustainable Energy Solution"`}
                    </p>

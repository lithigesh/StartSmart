...
const IdeathonDetailsPage = () => {
  ...
  const getStatusColor = (statusInfo) => {
    const colors = {
      blue: "bg-white/20/20 text-white/90 border-white/30",
      green: "bg-white/10/20 text-white/90 border-white/30",
      red: "bg-white/20 text-white/80 border-white/30",
      ...
      <div className="min-h-screen bg-black py-12 px-4 sm:px-6 lg:px-8">
        <div className="max-w-5xl mx-auto">
          <div className="bg-white/30 border border-white/30 rounded-lg p-6 text-center">
            <FaTimes className="text-5xl text-white/80 mx-auto mb-4" />
            <h2 className="text-2xl font-bold text-white mb-2">
              Error Loading Ideathon
            </h2>
        ...
        <div className="bg-gradient-to-br from-white/[0.08] to-white/[0.04] border border-white/10 rounded-2xl p-8 shadow-2xl">
          ...
          <div className="flex flex-wrap justify-between items-start gap-4 mb-6">
            <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-3 mb-4">
                ...
                {isRegistered && (
                  <span className="px-4 py-2 rounded-full text-sm font-semibold bg-white/10/20 text-white/90 border border-white/30 flex items-center gap-2">
                    <FaCheckCircle />
                    Registered
                  </span>
                )}
          ...
          {ideathon.theme && (
            <div className="mb-6 p-4 bg-white/5 border border-white/10 rounded-lg">
              <h2 className="text-white/70 text-sm font-semibold uppercase mb-1">
                Theme
              </h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            ...
            <div className="p-4 bg-white/5 border border-white/10 rounded-lg">
              <div className="flex items-center gap-2 mb-2">
                ...
                <h3 className="text-white/70 text-sm font-semibold uppercase">
                  Organizer
                </h3>
              </div>
            ...
            {ideathon.participantsCount !== undefined && (
              <div className="p-4 bg-white/5 border border-white/10 rounded-lg">
                ...
                <p className="text-white text-lg">
                  {ideathon.participantsCount} Teams
                </p>
              </div>
            )}
          </div>
          ...
          {/* Submission Format */}
          {ideathon.submissionFormat &&
            ideathon.submissionFormat.length > 0 && (
              <div className="mb-6 p-6 bg-white/5 border border-white/10 rounded-lg">
                <div className="flex items-center gap-2 mb-3">
                  <FaInfoCircle className="text-white/50" />
                  <h2 className="text-white/90 text-xl font-semibold">
                ...
                <div className="flex flex-wrap gap-2">
                  {ideathon.submissionFormat.map((format, index) => (
                    <span
                      key={index}
                      className="px-4 py-2 bg-white/20/20 text-white/90 border border-white/30 rounded-lg text-sm font-medium"
                    >
                      {format}
                    </span>
                  ))}
          ...
          {!isRegistered && statusInfo.status !== "expired" && (
            <div className="mt-8 flex justify-center">
              <button
                ...
                className="px-8 py-4 bg-white/20 text-white rounded-lg font-semibold text-lg hover:bg-white/30 transition-all duration-300 flex items-center gap-3 border border-white/30 shadow-lg"
              >
                <FaTrophy />
          ...
          {statusInfo.status === "expired" && !isRegistered && (
            <div className="mt-8 text-center">
              <div className="inline-flex items-center gap-2 px-6 py-3 bg-white/20 text-white/80 border border-white/30 rounded-lg">
                <FaClock />
                <span className="font-medium">
                  Registration has ended for this ideathon
                </span>
              </div>
            </div>
          )}

...
const SavedComparisonsSection = () => {
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-3 mb-6">
          <FaBalanceScale className="w-8 h-8 text-green-400" />
          <div>
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Error Message */}
      {error && (
        <div className="bg-red-500/20 border border-red-500/30 rounded-lg p-4 text-red-400 font-manrope">
          {error}
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        {comparisons.map((comparison, index) => (
          <div
            key={comparison._id}
            className="bg-gradient-to-br from-white/[0.08] to-white/[0.03] backdrop-blur-xl border-2 border-white/20 rounded-2xl p-6 hover:from-white/[0.12] hover:to-white/[0.06] hover:border-green-400/40 transition-all duration-300 group relative overflow-hidden hover:scale-105 hover:shadow-2xl hover:shadow-green-500/10 animate-fade-in"
            style={{ animationDelay: `${index * 100}ms` }}
          >
            {/* Animated Background */}
            <div className="absolute inset-0 bg-gradient-to-br from-green-500/[0.05] via-transparent to-emerald-500/[0.05] rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"></div>
            ...
            <div className="relative z-10 space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3 text-white/60">
                  <div className="bg-gradient-to-br from-green-500/20 to-emerald-500/20 p-2 rounded-lg border border-green-400/30">
                ...
                <div className="bg-gradient-to-r from-green-500/20 to-emerald-500/20 text-green-300 px-3 py-1.5 rounded-xl text-xs font-bold font-manrope border border-green-400/30 shadow-lg shadow-green-500/10">
                ...
                <div className="grid grid-cols-1 gap-2">
                  {comparison.ideas?.slice(0, 4).map((idea, ideaIndex) => (
                    <div
                      ...
                      className="bg-gradient-to-r from-white/[0.08] to-white/[0.04] border border-white/20 rounded-xl px-4 py-3 flex items-center gap-3 hover:from-white/[0.12] hover:to-white/[0.08] hover:border-green-400/30 transition-all duration-300 group/idea"
                    >
                      <div className="w-2 h-2 bg-green-400 rounded-full group-hover/idea:animate-pulse flex-shrink-0"></div>
                      ...
                      {comparison.chosenLeader?._id === idea._id && (
                        <div className="relative">
                          <div className="absolute inset-0 bg-orange-400/30 blur-lg rounded-full"></div>
                          <FaTrophy
                            className="relative w-4 h-4 text-orange-400 animate-bounce-subtle"
                            title="Chosen Leader"
                          />
                        </div>
                      )}
              ...
              {comparison.chosenLeader && (
                <div className="bg-gradient-to-r from-orange-500/20 to-amber-500/20 border-2 border-orange-400/40 rounded-xl p-4 backdrop-blur-sm relative overflow-hidden">
                  <div className="absolute inset-0 bg-gradient-to-br from-orange-400/10 to-transparent"></div>
                  <div className="relative flex items-start gap-3">
                    <div className="bg-orange-500/30 p-2 rounded-lg border border-orange-400/50 shadow-lg">
                      <FaTrophy className="w-5 h-5 text-orange-300" />
                    </div>
                    <div className="flex-1">
                      <div className="text-orange-300 font-manrope font-bold text-sm mb-1">
                        Chosen Leader
                      </div>
              ...
              {(comparison.notes || comparison.rationale) && (
                <div className="bg-gradient-to-br from-white/[0.06] to-white/[0.02] border border-white/20 rounded-xl p-4 backdrop-blur-sm">
                  <div className="flex items-center gap-2 text-white/80 font-manrope font-semibold text-sm mb-2">
                    <FaStickyNote className="w-4 h-4 text-blue-400" />
                    Notes
                  </div>
              ...
              <div className="flex items-center gap-3 pt-2">
                ...
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-yellow-500/20 to-orange-500/20 text-yellow-300 hover:from-yellow-500/30 hover:to-orange-500/30 rounded-xl transition-all duration-300 font-manrope font-bold min-h-[48px] touch-manipulation border-2 border-yellow-400/30 hover:border-yellow-400/50 hover:scale-105 hover:shadow-lg hover:shadow-yellow-500/20"
                >

...
const AnalyticsDashboard = () => {
  ...
  const fetchChartData = async () => {
    try {
      // Only fetch ideas chart data for the growth trends
      const queryParams = new URLSearchParams({
        type: 'ideas',
        dateRange: JSON.stringify(dateRange)
      });
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="bg-white/5 rounded-xl p-4 border border-white/10">
                <div className="flex flex-col space-y-2">
                  <span className="text-white/70 text-sm">Total Assessments</span>
                  <span className="text-white font-bold text-2xl">{analytics.sustainability?.totalAssessments || 0}</span>
                </div>
              </div>
              
              <div className="bg-white/5 rounded-xl p-4 border border-white/10">
                <div className="flex flex-col space-y-2">
                  <span className="text-white/70 text-sm">Avg Overall Score</span>
                  <span className="text-white font-bold text-2xl">{analytics.sustainability?.averageScore?.toFixed(1) || 0}/5</span>
                </div>
              </div>
              
              <div className="bg-white/5 rounded-xl p-4 border border-white/10">
                <div className="flex flex-col space-y-2">
                  <span className="text-white/70 text-sm">Environmental</span>
                  <span className="text-white font-bold text-2xl">{analytics.sustainability?.averageEnvironmental?.toFixed(1) || 0}/5</span>
                </div>
              </div>
              
              <div className="bg-white/5 rounded-xl p-4 border border-white/10">
                <div className="flex flex-col space-y-2">
                  <span className="text-white/70 text-sm">Social Impact</span>
                  <span className="text-white font-bold text-2xl">{analytics.sustainability?.averageSocial?.toFixed(1) || 0}/5</span>
                </div>
              </div>
            </div>
          </div>

...
const MyRegisteredIdeathons = () => {
  ...
  const fetchRegistrations = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_BASE}/api/ideathons/my-registrations`,
        {
          headers: {
  ...
  const handleWithdraw = async (registrationId, ideathonTitle) => {
    if (
      !window.confirm(
        `Are you sure you want to withdraw from "${ideathonTitle}"? This action cannot be undone.`
      )
    ) {
      return;
    }
    ...
    try {
      ...
      toast.error(
        error.response?.data?.message || "Failed to withdraw from ideathon"
      );
    }
  };
        ...
        {registrations.map((registration) => (
          ...
          >
            <div className="space-y-3">
              <div className="flex justify-between items-start">
                <h3 className="text-lg font-semibold text-white">
                  {registration.ideathon?.title}
                </h3>
                <span
                  className={`px-3 py-1 rounded-full text-xs font-semibold border whitespace-nowrap ${
                    registration.ideathon?.status === "ongoing"
                      ? "bg-white/10/20 text-white/90 border-white/30"
                      : registration.ideathon?.status === "completed"
                      ? "bg-white/20/20 text-white/90 border-white/30"
                      : "bg-white/20 text-white/70 border-white/30"
                  }`}
                >
                  {registration.ideathon?.status
                    ? registration.ideathon.status.charAt(0).toUpperCase() +
                      registration.ideathon.status.slice(1)
                    : "Unknown"}
                </span>
              </div>
              <div className="flex flex-wrap gap-2">
                ...
                <button
                  ...
                  className="px-3 py-1 bg-white/20 text-white/80 border border-white/30 rounded-full text-xs font-semibold hover:bg-white/30 transition-colors flex items-center gap-1 whitespace-nowrap"
                >
            ...
            <div className="text-sm text-white/60 mb-4">
              {registration.ideathon?.theme}
            </div>

            <button
              onClick={() =>
                setExpandedId(
                  expandedId === registration._id ? null : registration._id
                )
              }
            ...
            >
              ...
              <div className="text-white/80 bg-white/5 p-3 rounded-lg">
                ...
                {registration.ideathon?.startDate &&
                registration.ideathon?.endDate
                  ? `${new Date(
                      registration.ideathon.startDate
              ...
              {registration.ideathon?.fundingPrizes && (
                <div className="text-white/80 bg-white/5 p-3 rounded-lg">
                  <span className="font-medium text-white/90">Prize Pool:</span>{" "}
                  {registration.ideathon.fundingPrizes}
                </div>
              )}
                  ...
                  <div className="mt-2">
                    <div
                      className={`inline-block px-3 py-1 rounded-full text-sm ${
                        registration.progressStatus === "planning"
                          ? "bg-white/20/20 text-white/90 border border-white/30"
                          : registration.progressStatus === "development"
                          ? "bg-white/20 text-white/70 border border-white/30"
                          : registration.progressStatus === "testing"
                          ? "bg-white/20/20 text-white/90 border border-white/30"
                          : registration.progressStatus === "finalizing"
                          ? "bg-white/20 text-white/80 border border-white/30"
                          : "bg-white/10/20 text-white/90 border border-white/30"
                      }`}
                    >
                      {registration.progressStatus.charAt(0).toUpperCase() +
                        registration.progressStatus.slice(1)}
                    </div>
                  </div>
